...
Value evaluate_board(Position& p)
{
    Value staticEval = evaluate_static_score(p);
    ...
    // int psqtEval = EvaluatePsqtScore(p);
    // int attackEval = EvaluateAttackScore(p);
    return staticEval ;// + pawnStructsEval + mobilityEval + psqtEval + attackEval;

}


Value evaluate_board(Position& p, Color turn)
{
    if(turn == WHITE)
...
Value evaluate_board(Position& p, Color turn, int depth)
{
    if( (turn+depth) % 2 )
        return -evaluate_board(p);
    else
...
//     int result = 0;
    ...
    //         while(sq = pop_lsb(&bb))
    //         {
    //         // NOT ACCURATE - PSQT is mirror symetric, not rotation
    //             Square sqr = c == WHITE ? sq : SQ_H8-sq;
    //             Score sc = PSQT[c][sqr];
    //             Value v = c == WHITE ? Value(sc) : -Value(sc);
    //             result += v;
    //         }
    //     }
...
Value evaluate_attack_score(Position& p)
{
    ...
    for(Square sq=SQ_A1; sq<=SQ_H8;sq+=(Square)1)
    {
        Piece pc = p.piece_on(sq);
        if(pc != NO_PIECE)
        {
        Bitboard bb = p.attacks_from(Piece(pc),Square(sq));
        result += popcount<Full>(bb);}
    }
...
Value evaluate_static_score(Position& p)
{
    ...
    result  = 30000        * (popcount<Full>(wk) - popcount<Full>(bk))
            + queenWeight  * (popcount<Full>(wq) - popcount<Full>(bq))
            + rookWeight   * (popcount<Full>(wr) - popcount<Full>(br))
            + bishopWeight * (popcount<Full>(wb) - popcount<Full>(bb))
            + knightWeight * (popcount<Full>(wn) - popcount<Full>(bn))
            + pawnWeight   * (popcount<Full>(wp) - popcount<Full>(bp));

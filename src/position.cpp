...
bool Position::legal(Move m, Bitboard pinned) const
{
    ...
    if(pinned != pinned_pieces(sideToMove))
    {
        ofstream outfile;
        outfile.open("crashboard.log", std::ios_base::app);
        outfile << "SIDE TO MOVE: " << (sideToMove == WHITE ? "WHITE" : "BLACK")
        << endl 
        << this->pretty()
        << endl
        << "PINNED: " << endl
        << Bitboards::pretty(pinned)
        << "pinned_pieces(sideToMove): " << endl
        << Bitboards::pretty(pinned_pieces(sideToMove))
        << endl;
        //cout << "$$$ WAIT FOR THE CRASH !!! $$$" << endl;
        exit(-1);
    }
...
{
    assert(moved_piece(m) != NO_PIECE);
    //std::cout << "move: " << move_to_uci(m,false) << std::endl;
    assert(is_ok(m));
...
void Position::do_move(Move m, StateInfo& newSt, const CheckInfo& ci, bool moveIsCheck)
{
    ...
    if (captured)
    {
        ...
        // prefetch((char*)thisThread->materialTable[st->materialKey]); ////////// NOT IN USE FOR NOW
    ...
    //prefetch((char*)TT.first_entry(k)); ////////// NOT IN USE FOR NOW
    ...
    if (pt == PAWN)
    {
        ...
        //prefetch((char*)thisThread->pawnsTable[st->pawnKey]);  ////////// NOT IN USE FOR NOW
    ...
    assert(pos_is_ok());
    assert(piece_on(from) == NO_PIECE || type_of(m) == PROMOTION);
}
...
void Position::undo_move(Move m)
{
    ...
    //     // assertion failed
    //     cout << "info assertion failed (empty(from) || type_of(m) == CASTLING)" << endl;
    //     cout << "info move to undo " << move_to_uci(m,false) << endl;
    //     cout << "info empty(from) " << (empty(from)? "TRUE":"FALSE")<< endl;
    //     cout << "info type_of(m) == CASTLING " << (type_of(m) == CASTLING? "TRUE":"FALSE")<< endl;
    //     cout << "info move to undo: " << pretty(m) << endl;
    //     cout << "info " << this->pretty() << endl;
    //     exit(-1);
    // }
...
void Position::do_null_move(StateInfo& newSt)
{
    ...
    st->key ^= Zobrist::side;
    //prefetch((char*)TT.first_entry(st->key)); ////////// NOT IN USE FOR NOW
...
std::string pretty(StateInfo& st) {
    stringstream ss;

    ss << "capturedType: " << st.capturedType << std::endl
        << "capturedType: " << st.capturedType << std::endl
        << "capturedType: " << st.capturedType << std::endl
        << "capturedType: " << st.capturedType << std::endl
        << "capturedType: " << st.capturedType << std::endl
        << "capturedType: " << st.capturedType << std::endl

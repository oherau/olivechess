...
ExtMove* generate_castling(const Position& pos, ExtMove* mlist, Color us, const CheckInfo* ci)
{
    ...
    assert(pos.piece_on(kfrom) != NO_PIECE);
...
ExtMove* generate<LEGAL>(const Position& pos, ExtMove* mlist)
{
    ...
    end = pos.checkers() ? generate<EVASIONS>(pos, mlist) : generate<NON_EVASIONS>(pos, mlist);
    while (cur != end)
...
inline bool IsFiftyMovesDraw(Position& pos)
{
    return (pos.game_ply() >= 50);
}


inline bool contains_promotion(ExtMove* moves)
{
    for(ExtMove* mv=moves; mv->move!=MOVE_NONE; mv++)
    {
        if(type_of(mv->move) == PROMOTION)
            return true;
    }
...
inline bool contains_move(ExtMove* moves, Move m)
{
    for(ExtMove* mv=moves; mv->move!=MOVE_NONE; mv++)
    {
        if(mv->move == m )
...
{
    for(ExtMove* mv=list; mv->move!=MOVE_NONE; mv++)
    {
        mv->value = -potentialValue(mv->move);
    }
}
...
//         for(ExtMove* mv=list; mv->move!=MOVE_NONE; mv++)
//         {
//             mv->value = MvvLvaValue(p,mv->move);
//         }
//         */
// }
...
//     for(ExtMove* mv=list;mv->move!=MOVE_NONE;mv++)
//     {
//     mv->value = -(Value)history[p.GetPiece(from_sq(mv->move))][to_sq(mv->move)];
//     }
//     */
// }
...
//     /*
//         // best captures first
//         Value captureEval = Value(2000*GetVal(pos.GetPiece(to_sq(move))));
//         // lowest piece moved for capture first
//         Value pieceEval = Value(2000-GetVal(pos.GetPiece(from_sq(move))));
...
{
    return Value(((move & 07777) << 2) | ((move >> 12) & 3));
}
